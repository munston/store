...
globalProjectHeader = "starship"

globalPath = "C://Users//HP//Desktop//project-starship//stores" +/+ globalProjectHeader
...
storeDirectory :: forall a. Store a => FilePath
storeDirectory  = globalPath +/+ name 
 where
  name = storeName @a

storePath :: forall a. Store a => Int -> FilePath
storePath i = storeDirectory @a +/+ (name ++ "." ++ (show i) ++ ".store")
 where
  name = storeName @a
...
startStore :: forall a. Store a => IO ()
startStore = do
  b <- doesStoreExist @a 0
  if not b then writeStore @a 0 storeInitialization else return ()
...
cleanEmptyStores = return () -- print "refuses to clean stores!!"
