...
stationaryCarrage :: forall (s::Nat) (c::Nat) (l::Nat) (e::Nat) a. (UpdateIndicies (Nexts c l) a,LoadIndicies (Nexts c l) a,SaveIndicies (Nexts c l) a) => (forall s'. Carrage s' c l e a -> IO [[Double]]) -> IO ()
...
type family InfiniteConstraint (s :: Nat) (c :: Nat) (l :: Nat) (e :: Nat) a :: Constraint where
 InfiniteConstraint s c l e a = If (e == (S c)) (() :: Constraint) (InfiniteConstraint s (S c) l e a)
...
instance Recurse s c l e a where
 recurse f = return () -- movingCarrage @s @(S c) @l @e @a f
