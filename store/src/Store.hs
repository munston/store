...
{-
storeName'' :: forall a. Store a => String
storeName'' = "store//"++ (storeName @a)
-}
...
storePath i = name +/+ (name ++ "." ++ (show i) ++ ".store")
 where
  name = storeName'' @a                                            -- alert! uses '' by hack
...
lastStoreNumber = do
 ...
 if not b then return 0 else do
  ...
  return $ (length xs) - 3 -- -2 as removes '.','..' paths -- note is -3 not -2 as last store number is 0
   where
    name = storeName @a
